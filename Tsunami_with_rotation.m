...
A=20;                                        % Amplitude of surface displacement (m)
La=800000;                                  % Horizontal scale of the earthquake ()
...
%%%%%%%%%%%%%%%%% Boundary conditions at first temporal step    
    ...
    z(n+1,Jx)=0;                         % Right boundary on the shelf j=Jx  
             ...
             %% Block5:  First step CTCS: n>1
    for n=2:Nt-1
       
    for j=2:Jx-1                      % Loop for u,v,z at first time dtep
       u(n+1,j)=u(n-1,j)-2*g*dt*(z(n,j+1)-z(n,j-1))/2/dx;
       z(n+1,j)=z(n-1,j)-2*dt*(H(j+1)*u(n,j+1)-H(j-1)*u(n,j-1))/2/dx;
    end                          % end of the loop
%%%%%%%%%%%%%%%%% Boundary conditions at first temporal step    
    u(n+1,1)=0;                          % In the epicentre j=1
    z(n+1,1)=z(n+1,2);                   % In the epicentre j=1 
...
fig1=figure(1);
set(gcf,'Units','Normalize','Position',[0.2,0.2,0.7,0.5]); %size and position of figure
...
ylim([-A 1.3*A]);                            % Limits for y-axis    
...
%%%%%%%%%%%%%%%%% Boundary conditions at first temporal step    
 ...
 %% Block 8 Visualization of nonlinear model
 
 subplot (2,1,1)
 hold on
 plot(x/1000,z(i,:),'LineWidth',2,'Color','r');
 
 
            %%%%%%%%%%%%%%%%%%% End of block 8
              

        %% Block9: Rotation** First step CTCS: n>1
    for j=1:Jx
        v(1,j)=0;
        v(2,j)=0;
    end
    ...
    for n=2:Nt-1
    for j=2:Jx-1                      % Loop for u,v,z at first time dtep
           u(n+1,j)=u(n-1,j)+2*f*v(n,j)*dt-2*g*dt*(z(n,j+1)-z(n,j-1))/2/dx-u(n,j)*(u(n,j+1)-u(n,j-1))*dt/dx;
           v(n+1,j)=v(n-1,j)-2*f*u(n,j)*dt-u(n,j)*(v(n,j+1)-v(n,j-1))*dt/dx;
           z(n+1,j)=z(n-1,j)-2*dt*((H(j+1)+z(n,j+1))*u(n,j+1)-(H(j-1)+z(n,j-1))*u(n,j-1))/2/dx;
    end                          % end of the loop
    %%%%%%%%%%%%%%%%% Boundary conditions at first temporal step    
        u(n+1,1)=0;                          % In the epicentre j=1
        ...
        u(n+1,Jx)=0;                         % Right boundary on the shelf j=Jx
        z(n+1,Jx)=0;                         % Right boundary on the shelf j=Jx  
        v(n+1,1)=0;                          % In the epicentre j=1
        v(n+1,Jx)=0;                         % Right boundary on the shelf j=Jx  
              ...
              %%%%%%%%%%%%%%%%  End of Block 9  
               ...
               %% Block 10 Visualization of nonlinear model
 
 subplot (2,1,1)
 hold on
 plot(x/1000,z(i,:),'LineWidth',2,'Color','g');
